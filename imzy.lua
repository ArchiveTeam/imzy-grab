...
allowed = function(url, parenturl)
  ...
  if string.match(url, "^https?://[^/]*imgix%.net")
     or string.match(url, "^https?://images%.imzy%.com") then
    return true
  end

  if string.match(url, "^https?://[^/]*imzy%.com") then
    if string.match(url, "^https?://[^/]*imzy%.com/@") then
      if ids[string.match(url, "^https?://[^/]*imzy%.com/@([^%?]+)")] == true then
        return true
      end
    end
    for id in string.gmatch(url, "([^/]+)") do
      if ids[id] == true then
        return true
      end
    end
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if allowed(url, nil) and not (string.match(url, "^https?://[^/]*imgix%.net")
     or string.match(url, "^https?://images%.imzy%.com")) then
    html = read_file(file)
