...
LUA_SHA1 = get_hash(os.path.join(CWD, 'imzy.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--warc-header", "imzy-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("imzy-item: %(item_name)s"),
        ]
        ...
        if item_type == 'user':
            wget_args.extend(['--warc-header', 'imzy-user: {item_value}'.format(item_value=item_value)])
            wget_args.append('https://www.imzy.com/api/accounts/profiles/{item_value}'.format(item_value=item_value))
            wget_args.append('https://www.imzy.com/api/accounts/profiles/{item_value}?check=true'.format(item_value=item_value))
            wget_args.append('https://www.imzy.com/@{item_value}'.format(item_value=item_value))
            wget_args.append('https://www.imzy.com/@{item_value}?tab=comments'.format(item_value=item_value))
            wget_args.append('https://www.imzy.com/@{item_value}?tab=posts'.format(item_value=item_value))
            wget_args.append('https://www.imzy.com/@{item_value}?tab=communities'.format(item_value=item_value))
            for i in [1, 0]:
            ...
            #wget_args.append('https://www.imzy.com/api/communities/{s}/comments?sort=popular&per_page=25'.format(s='/posts/'.join(item_value.split(':', 1))))
...
project = Project(
    title = "imzy",
    project_html = """
    <img class="project-logo" alt="logo" src="https://images.imzy.com/web/home/home-logo-green.png" />
    <h2>imzy.com <span class="links"><a href="http://imzy.com/">Website</a> &middot; <a href="http://tracker.archiveteam.org/imzy/">Leaderboard</a></span></h2>
    """,
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="imzy"),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
